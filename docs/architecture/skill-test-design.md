# Skill Test Design Architecture

## Overview

The skill-test-design skill guides incremental creation and refinement of `*.spec.md` test files for AI agent skills. It reads a target skill's SKILL.md, asks targeted questions about gaps, and generates test cases by category with refinement loops.

Diagnosing why an existing test fails or behaves flakily is a separate concern owned by the `skill-test-troubleshooting` skill. The two skills are intentionally split so each has a sharp, distinct activation surface: authoring vs. diagnosis.
...
The most common test failure mode is the skill not activating. The agent receives the prompt but never invokes the skill-test-design skill, instead trying to handle the task on its own. Symptoms: the transcript shows no `Skill` tool call, and the agent spends turns exploring the codebase or writing tests from scratch.

The fix is always to rephrase the description. See `skills/skill-test-troubleshooting/references/troubleshooting.md` for the general playbook (it applies to any skill, not just this one).
...
This is documented in `skills/skill-test-design/references/fixture-design.md` and enforced by test cases in the skill-test-design suite (TD-4).

## Prompt Design for Single-Turn Tests

Many skill-test-design behaviors involve multi-step interactive flows (approval gates, one question at a time, incremental category generation). In single-turn tests, we can only verify the first response.
...
The skill-test-design tests are organized across multiple spec files by concern:

| File                            | Prefix | Concern                                                                                              |
| ------------------------------- | ------ | ---------------------------------------------------------------------------------------------------- |
| `skill-test-design.spec.md`     | `TD`   | Core functionality: test case quality, existing spec detection, malformed skills, fixture neutrality |
| `skill-test-design-pdd.spec.md` | `PDD`  | Prompt-driven development: nonexistent skills, capability mismatches, redundant questions            |
