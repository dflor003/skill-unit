...
// Logger writes to stderr; markdown formatter writes to stdout. Check both.
function shouldColor(stream) {
  return !process.env.NO_COLOR && stream.isTTY;
}

const useColorStderr = shouldColor(process.stderr);
const useColorStdout = shouldColor(process.stdout);

function c(color, text) {
  if (!useColorStderr) return text;
...
// Update the log level for all existing and future logger instances.
// Call after loading config: require("./logger").setLevel("verbose")
...
//
// Two modes:
//   formatMd(text)              — format a complete markdown string
//   new MdStream(stream)        — streaming formatter for chunked text
//     .write(chunk)             — process a chunk (may contain partial lines)
//     .end()                    — flush remaining buffer

function formatLine(line, inCodeBlock, useColor) {
...
function formatInline(text, useColor) {
  ...
  result = result.replace(/(?<!_)_([^_]+)_(?!_)/g, (_, it) => {
    return `${ANSI.italic}${it}${ANSI.reset}`;
  });
...
// Format a complete markdown string (batch mode).
function formatMd(text, stream) {
