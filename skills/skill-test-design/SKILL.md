...
- **Slash command:** `/skill-test-design`, `/skill-test-design <skill-name>`
...
2. skill-test-design (plugin: skills/skill-test-design/SKILL.md)
...
This skill operates on a single target skill per invocation. If the user wants to design tests for multiple skills, they invoke `/skill-test-design` separately for each.
...
If the skill uses Read, Write, Edit, Glob, or Grep on project files, or references specific file types or directory structures, or depends on git state — note that **fixtures will likely be needed**. Use the Read tool to load this skill's `references/fixture-design.md` (path: `skills/skill-test-design/references/fixture-design.md` from repo root) for guidance on fixture design and incorporate fixture questions into the targeted questions below.
