...
export const testCommand = defineCommand({
  ...
  args: {
    ...
    all: {
      type: 'boolean',
      description:
        'Run all tests (required when no other filters are provided)',
    },
    file: {
      type: 'string',
      alias: 'f',
  ...
  async run({ args, rawArgs }) {
    ...
    log.info(
      `Compiled ${manifests.length} manifest${manifests.length === 1 ? '' : 's'}`
    );
    for (const manifest of manifests) {
      log.verbose(
        `  ${manifest['spec-name']}: ${manifest['test-cases'].length} test case${manifest['test-cases'].length === 1 ? '' : 's'}`
      );
    }
    ...
    log.info(
      `Running ${allTasks.length} test${allTasks.length === 1 ? '' : 's'} (concurrency: ${concurrency})`
    );
    ...
    const testRunResults: TestRunResult[] = await Promise.all(
      allTasks.map(async ({ manifest, testCase }) => {
        ...
        try {
          ...
          log.error(
            `[${testCase.id}]: ${err instanceof Error ? err.message : String(err)}`
          );
          return {
            manifest,
            testCase,
    ...
    const testResults: TestResult[] = testRunResults.map((tr) => {
      ...
      return {
        ...
        transcriptPath: path.join(
          runDir,
          'results',
          `${specName}.${tr.testCase.id}.transcript.md`
        ),
        resultPath: graded
          ? path.join(runDir, 'results', graded.fileName)
          : undefined,
      };
    });
